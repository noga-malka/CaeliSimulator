...
def update_profile_grid(*inputs: list):
    cards = []
    for profile in DatabaseManager().profile_manager.get_instances():
        cards.append(create_grid_card(profile.name, generate_profile_details(profile), 'profile_manager', False))
    return cards


@callback(Output(Placeholder.ID, Placeholder.Fields.TAB_INDEX),
          Input({'index': ALL, 'type': 'profile_manager'}, 'n_clicks'),
